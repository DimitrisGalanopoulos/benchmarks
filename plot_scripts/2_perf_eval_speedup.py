...
def do_plot(dfs, file_out):
    ...
    df = calculate_gmeans(df, 'speedup')

    p = Barplot(data=df, x='matrix_id', y='speedup',  hue='format_name', palette=G.palette_format_dict)
    ...
    p.ax.grid(axis='y', color='black', linestyle=(0, (50,50)), linewidth=0.2)
...
file_out = 'figures/2_performance_speedup_barplot_amd.pdf'
...
file_out = 'figures/2_performance_speedup_barplot_intel.pdf'
df, p = do_plot(dfs, file_out)
p.plot(file_out)
