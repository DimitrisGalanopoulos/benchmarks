...
df = df[['matrix_id', 'matrix_name', 'nnz', 'csr_mem_footprint', 'UV', 'mem_ratio']].reset_index(drop=True)
df['mem_ratio'] = df['mem_ratio'] * 100
df['nnz'] = round(df['nnz'] / 1000000)
...
df['nnz'] = df['nnz'].map(lambda x: '%dM' % x)

df.to_latex('matrix_suite_more.csv', index=False)
print(df)
